                    ...
                    let handles: Vec<_> = (0..concurrent_count)
                        .map(|i| {
                            ...
                            tokio::spawn(async move {
                                let doc =
                                    create_test_document_with_id(&format!("concurrent_{}", i));
                                cluster.insert_document(doc).await.unwrap()
                            })
                        })
                        .collect();
    ...
    group.bench_function("six_node_scalability", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            let handles: Vec<_> = docs
                ...
                .map(|doc| {
                    let cluster = cluster.clone();
                    tokio::spawn(async move { cluster.insert_document(doc).await })
                })
                .collect();
...
async fn execute_grpc_operation(
    cluster: &TestCluster,
...
async fn simulate_rest_bulk_insert(
    cluster: &TestCluster,
