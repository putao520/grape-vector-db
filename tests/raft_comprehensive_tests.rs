...
mod raft_basic_tests {
    ...
    async fn test_log_replication() {
        ...
        for entry in &entries {
            leader.propose_entry(entry.clone()).await
                .expect("日志提议应该成功");
        }
        ...
        // 验证所有节点日志一致
        assert!(cluster.verify_log_consistency().await, "所有节点日志应该一致");
    ...
    async fn test_concurrent_log_entries() {
        ...
        // 验证日志一致性
        assert!(cluster.verify_log_consistency().await, "并发日志条目应该保持一致");
...
mod raft_fault_tolerance_tests {
    ...
    async fn test_leader_failure_recovery() {
        ...
        // 验证新领导者不是原来的领导者
        assert_ne!(new_leader.node_id(), original_leader_id, "应该选出新的领导者");
    ...
    async fn test_follower_failure_recovery() {
        ...
        for i in 0..5 {
            ...
            leader.propose_entry(entry).await
...
mod raft_network_partition_tests {
    ...
    async fn test_partition_with_leader_in_minority() {
        ...
        cluster.create_partition(vec![0], vec![1, 2, 3, 4, 5]).await.unwrap();
        ...
        // 验证网络恢复后的一致性
        assert!(cluster.can_reach_consensus().await, "网络恢复后应该达成共识");
...
mod raft_edge_case_tests {
    ...
    async fn test_rapid_leadership_changes() {
        ...
        for i in 0..5 {
            ...
            // 验证集群仍然可用
            assert!(cluster.is_available().await, "第{}次领导者变更后集群应该可用", i + 1);
        }
...
mod raft_performance_tests {
    ...
    async fn test_high_throughput_log_replication() {
        ...
        assert!(avg_latency <= Duration::from_millis(100), "平均延迟应该 <= 100ms");
    ...
    async fn test_long_running_consensus() {
        ...
        assert!(cluster.verify_log_consistency().await, "长时间运行后日志应该一致");
        assert!(cluster.is_available().await, "集群应该仍然可用");
