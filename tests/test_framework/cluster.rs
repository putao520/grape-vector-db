...
use crate::test_framework::{ChaosEngine, NetworkSimulator, TestNode};
...
impl TestCluster {
    ...
    pub async fn create_partition(
        ...
        // 获取分区1和分区2的节点ID
        let partition1_ids: Vec<String> = partition1
            .iter()
            .filter_map(|&i| nodes.get(i).map(|n| n.node_id().to_string()))
            .collect();
        let partition2_ids: Vec<String> = partition2
            .iter()
            .filter_map(|&i| nodes.get(i).map(|n| n.node_id().to_string()))
            .collect();
        ...
        self.network_simulator
            .create_partition(partition1_ids, partition2_ids)
            .await;
