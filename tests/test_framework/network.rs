...
impl NetworkSimulator {
    ...
    pub async fn get_largest_partition(&self) -> Option<Partition> {
        let partitions = self.partitions.read().await;
        partitions.iter().max_by_key(|p| p.nodes.len()).cloned()
    }
            ...
            match operation {
                ...
                1 => {
                    ...
                    if fastrand::f64() < 0.05 {
                        ...
                        self.set_latency(node1.clone(), node2.clone(), spike_latency)
                            .await;
                        ...
                        tokio::spawn(async move {
                            ...
                            simulator
                                .set_latency(node1_clone, node2_clone, Duration::from_millis(1))
                                .await;
                        });
                    }
                }
...
pub mod network_test_utils {
    ...
    /// 创建多数派-少数派分区
    pub async fn create_majority_minority_partition(
        ...
        let majority: Vec<String> = (minority_size..total_nodes)
    ...
    /// 随机失败节点
    pub async fn random_node_failures(
        simulator: &NetworkSimulator,
        node_count: usize,
        failure_rate: f64,
    ) {
...
mod tests {
    ...
    async fn test_network_chaos() {
        ...
        // 运行短时间的混沌测试
        simulator
            .inject_chaos(chaos, Duration::from_millis(100))
            .await
            .unwrap();
