...
use crate::test_framework::{NetworkChaos, NetworkSimulator};
...
impl ChaosEngine {
            ...
            loop {
                ...
                if fastrand::f64() < network_failure_rate {
                    if let Some(chaos) = &network_chaos {
                        let _ = network_simulator
                            .inject_chaos(chaos.clone(), Duration::from_millis(200))
                            .await;
                    }
                }
    ...
    async fn collect_experiment_metrics(&self) -> ExperimentMetrics {
        ...
        let available_points = collector
        ...
        let avg_read_latency = if collector.performance_metrics.is_empty() {
            ...
            collector
                ...
                .sum::<f64>()
                / collector.performance_metrics.len() as f64
        };
        ...
        // 计算一致性违反
        let consistency_violations = collector
            .consistency_metrics
            .iter()
