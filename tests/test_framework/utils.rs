...
/// 创建具有特定属性的测试文档
pub fn create_test_document_with_metadata(id: &str, metadata: HashMap<String, String>) -> Document {
    Document {
        id: id.to_string(),
        title: Some(format!("测试文档 {}", id)),
        content: format!("这是测试文档{}的内容", id),
...
impl LoadBalanceValidator {
    ...
    pub fn get_distribution(&self) -> HashMap<String, f64> {
        ...
        self.node_counts
            .iter()
            .map(|(node, count)| (node.clone(), *count as f64 / self.total_operations as f64))
            .collect()
    }
}
