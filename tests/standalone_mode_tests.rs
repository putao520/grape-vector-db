...
mod standalone_service_tests {
    ...
    async fn test_rest_api_compatibility() {
        ...
        let search_result =
            simulate_rest_search(&cluster, "test_collection", query_vector, 5).await;
    ...
    async fn simulate_grpc_batch_insert(
        cluster: &TestCluster,
        docs: Vec<Document>,
    ) -> Result<Vec<String>> {
        // 模拟gRPC批量插入
    ...
    async fn simulate_rest_insert_points(
        cluster: &TestCluster,
        collection: &str,
...
mod standalone_persistence_tests {
    ...
    async fn test_index_recovery() {
        ...
        // 插入带向量的文档
        let docs_with_vectors: Vec<Document> = (0..30)
            .map(|i| {
                create_test_document_with_vector(
                    &format!("vector_doc_{}", i),
                    generate_test_vector(i),
                )
            })
            .collect();
        ...
        // 执行搜索以确保索引正常工作
        let search_results = simulate_vector_search(&cluster, &generate_test_vector(0), 5)
            .await
            .unwrap();
        ...
        // 验证索引恢复
        let recovery_results = simulate_vector_search(&cluster, &generate_test_vector(0), 5)
            .await
            .unwrap();
    ...
    // 辅助函数
    async fn simulate_vector_search(
        cluster: &TestCluster,
        query_vector: &[f32],
        limit: usize,
    ) -> Result<Vec<SearchResult>> {
...
mod standalone_performance_tests {
    ...
    async fn test_single_node_throughput() {
        ...
        // 并发写入测试
        let write_handles: Vec<_> = (0..operation_count)
            .map(|i| {
        ...
        let read_handles: Vec<_> = (0..operation_count)
            .map(|i| {
                let cluster = cluster.clone();
                tokio::spawn(async move {
        ...
        assert!(
            avg_write_latency <= Duration::from_millis(50),
            "写入延迟应该 <= 50ms"
        );
        assert!(
            avg_read_latency <= Duration::from_millis(20),
            "读取延迟应该 <= 20ms"
        );
    ...
    async fn test_search_performance() {
        ...
        for (scenario_name, query, search_type) in search_scenarios {
            ...
            let search_handles: Vec<_> = (0..search_count)
                .map(|_| {
                    let cluster = cluster.clone();
            ...
            // 搜索性能断言
            assert!(
                search_throughput >= 200.0,
                "{}吞吐量应该 >= 200 searches/sec",
                scenario_name
            );
            assert!(
                avg_search_latency <= Duration::from_millis(100),
                "{}延迟应该 <= 100ms",
                scenario_name
            );
        }
    ...
    fn generate_small_documents(count: usize) -> Vec<Document> {
        (0..count)
            .map(|i| create_test_document_with_id(&format!("small_doc_{}", i)))
            .collect()
    }
    ...
    fn generate_large_documents(count: usize) -> Vec<Document> {
        (0..count)
            .map(|i| {
                ...
                doc.content = format!(
                    "这是一个大型文档的内容，包含大量的文字信息用于测试内存使用效率。{}",
                    "重复内容 ".repeat(100)
                );
