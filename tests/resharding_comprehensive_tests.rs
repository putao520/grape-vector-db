...
mod shard_performance_tests {
    ...
    async fn test_large_scale_resharding() {
        ...
        for i in 0..doc_count {
            batch_docs.push(create_test_document_with_id(&format!(
                "large_scale_doc_{}",
                i
            )));
        ...
        let resharding_task: tokio::task::JoinHandle<Result<(), anyhow::Error>> = tokio::spawn(async {
            ...
            // 并行迁移数据
            let migration_tasks: Vec<_> = (0..10)
                .map(|i| {
        ...
        // 性能和完整性断言
        assert!(
            resharding_duration <= Duration::from_secs(2),
            "大规模重分片应该在2秒内完成"
        );
