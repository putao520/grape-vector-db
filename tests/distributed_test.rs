...
use grape_vector_db::distributed::shard::{ConsistentHashRing, ShardConfig, ShardManager};
...
async fn test_consistent_hash_ring() {
    ...
    assert_eq!(
        stats.get("physical_nodes_count").unwrap().as_u64().unwrap(),
        3
    );
    assert_eq!(
        stats.get("virtual_nodes_count").unwrap().as_u64().unwrap(),
        300
    );
...
async fn test_shard_manager_initialization() {
    ...
    shard_manager
        .initialize_shards(cluster_nodes)
        .await
        .expect("分片初始化失败");
...
async fn test_shard_health_monitoring() {
    ...
    // 收集健康状态
    let health_statuses = shard_manager
        .collect_all_shard_health()
        .await
        .expect("收集健康状态失败");
    ...
    // 获取集群负载统计
    let cluster_stats = shard_manager
        .get_cluster_load_stats()
        .await
        .expect("获取集群统计失败");
...
async fn test_shard_rebalancing() {
    ...
    shard_manager
        .initialize_shards(cluster_nodes.clone())
        .await
        .expect("分片初始化失败");

    // 测试重平衡
    let migrations = shard_manager
        .rebalance_shards(cluster_nodes)
        .await
        .expect("重平衡失败");
