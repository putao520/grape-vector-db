...
mod embedded_basic_tests {
    ...
    async fn test_embedded_startup_shutdown() {
        ...
        // 测试启动
        let db = VectorDatabase::new(temp_dir.path().to_path_buf(), config).await
            .expect("内嵌数据库启动应该成功");
        ...
        assert!(shutdown_time <= Duration::from_millis(500), "关闭时间应该 <= 500ms");
    ...
    async fn test_embedded_lifecycle_management() {
        ...
        // 第一次创建和使用
        {
            let db = VectorDatabase::new(temp_dir.path().to_path_buf(), config.clone()).await.unwrap();
...
mod embedded_performance_tests {
    ...
    async fn test_embedded_memory_usage() {
        ...
        println!("  平均每文档: {:.2} KB", (memory_increase * 1024.0) / doc_count as f64);
    ...
    async fn test_embedded_concurrent_access() {
        ...
        // 性能断言
        assert!(write_success >= concurrent_count * 95 / 100, "写入成功率应该 >= 95%");
        assert!(read_success >= concurrent_count * 95 / 100, "读取成功率应该 >= 95%");
    ...
    async fn test_embedded_large_dataset() {
        ...
        assert!(rebuild_duration <= Duration::from_secs(10), "索引重建应该 <= 10s");
        assert!(search_duration <= Duration::from_millis(100), "搜索时间应该 <= 100ms");
...
mod embedded_concurrency_tests {
    ...
    async fn test_embedded_thread_safety() {
        ...
        println!("  成功率: {:.1}%", success_count as f64 / total_operations as f64 * 100.0);
        
        // 线程安全断言
        assert!(success_count + error_count == total_operations, "操作计数应该正确");
        assert!(success_count >= total_operations * 95 / 100, "成功率应该 >= 95%");
...
mod embedded_resource_tests {
    ...
    async fn test_embedded_resource_cleanup() {
        ...
        // 验证可以继续正常使用
        let final_db = VectorDatabase::new(temp_dir.path().to_path_buf(), config).await.unwrap();
    ...
    async fn test_embedded_memory_leak_prevention() {
        ...
        // 内存泄漏预防断言
        assert!(memory_growth <= 50.0, "内存增长应该在合理范围内: {:.1} MB", memory_growth);
