...
use grape_vector_db::{
    errors::Result,
    filtering::{
        ComparisonOperator, FilterConfig, FilterEngine, FilterExpression, FilterMetrics,
        FilterValue, GeometryValue, GeospatialOperator, LogicalOperator, NestedOperator,
        SqlFilterParser, TextSearchOptions,
    },
};
...
fn benchmark_filter_performance(engine: &FilterEngine) -> Result<()> {
    ...
    for (name, filter) in test_filters {
        ...
        let avg_results = if success_count > 0 {
