...
fn demo_basic_operations(storage: &AdvancedStorage) -> Result<()> {
    ...
    for i in 0..100 {
        ...
        payload.insert(
            "category".to_string(),
            serde_json::Value::String(format!("cat_{}", i % 5)),
        );
        payload.insert(
            "score".to_string(),
    ...
    println!(
        "   ✅ Retrieved {} vectors in {:?}",
        retrieved_count, retrieve_duration
    );
...
fn demo_transaction_support(storage: &AdvancedStorage) -> Result<()> {
    ...
    for i in 200..250 {
        ...
        payload.insert(
            "batch_id".to_string(),
            serde_json::Value::String("batch_1".to_string()),
        );
        payload.insert(
            "index".to_string(),
    ...
    println!(
        "   ✅ Verified {} vectors with correct batch_id in {:?}",
        verified_count, verify_duration
    );
...
fn demo_backup_recovery(storage: &AdvancedStorage) -> Result<()> {
    ...
    println!(
        "      Compression ratio: {:.2}",
        initial_stats.compression_ratio
    );
    ...
    println!(
        "   ✅ Created checkpoint at {:?} in {:?}",
        checkpoint_path, checkpoint_duration
    );
...
fn demo_performance_benchmarks(storage: &AdvancedStorage) -> Result<()> {
    ...
    println!(
        "   ✅ Batch write: {} vectors in {:?} ({:.0} QPS)",
        vector_count, write_duration, write_qps
    );
    ...
    println!(
        "   ✅ Random reads: {}/{} vectors in {:?} ({:.0} QPS)",
        successful_reads, read_count, read_duration, read_qps
    );
...
fn demo_multi_tree_usage(storage: &AdvancedStorage) -> Result<()> {
    ...
    for i in 0..10 {
        ...
        payload.insert(
            "tree_demo".to_string(),
            serde_json::Value::String("multi_tree".to_string()),
        );
    ...
    println!(
        "   ✅ Retrieved {} vectors from multi-tree storage",
        retrieved
    );
