...
use grape_vector_db::{
    errors::Result,
    quantization::{
        BinaryQuantizationConfig, BinaryQuantizer, BinaryVectorStore, QuantizationMetrics,
    },
};
...
fn main() -> Result<()> {
    ...
    for i in 0..num_queries {
        let binary_top5: std::collections::HashSet<usize> =
            binary_results[i].iter().map(|(idx, _)| *idx).collect();
        let exact_top5: std::collections::HashSet<usize> =
            exact_results[i].iter().map(|(idx, _)| *idx).collect();
    ...
    if !queries.is_empty() && !dataset.is_empty() {
        ...
        let start = Instant::now();
        let multi_stage_results =
            quantizer.multi_stage_search(&query_binary, &binary_dataset, query, &dataset)?;
