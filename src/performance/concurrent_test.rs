use crate::{concurrent::*, Document, VectorDatabase, VectorDbConfig};
use std::sync::Arc;
...
impl ConcurrentPerformanceTest {
    ...
    /// 并发插入性能测试
    pub async fn test_concurrent_insertions(
        &self,
        batch_count: usize,
        batch_size: usize,
    ...
    /// 并发搜索性能测试
    pub async fn test_concurrent_searches(
        &self,
        concurrent_searches: usize,
        searches_per_thread: usize,
    ) -> Result<f64, Box<dyn std::error::Error>> {
        println!(
            "🔍 测试并发搜索性能：{} 个并发线程，每线程 {} 次搜索",
            concurrent_searches, searches_per_thread
        );
        ...
        println!(
            "   缓存命中率: {:.1}%",
            self.counters.cache_hit_rate() * 100.0
        );
        println!("   成功率: {:.1}%", self.counters.success_rate() * 100.0);
    ...
    /// 混合工作负载测试
    pub async fn test_mixed_workload(
        &self,
        duration_secs: u64,
    ...
    pub async fn run_full_suite(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "   总体成功率: {:.1}%",
            self.counters.success_rate() * 100.0
        );
...
mod tests {
    ...
    async fn test_concurrent_performance_suite() {
        ...
        println!(
            "并发性能测试通过：插入 {:.2} doc/s, 搜索 {:.2} search/s",
            insert_perf, search_perf
        );
    }
}
